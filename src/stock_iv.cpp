...
namespace std {
    ostream &operator << (ostream &os, const Segment &seg) {
        os << "( " << seg.start << ", " << seg.stop << " )";
        return os;
    }

    ostream &operator << (ostream &os, const vector<Segment> &segs) {
        os << "{ ";
        string comma = "";
...
#ifdef RUN_TEST
TEST_CASE("188. Best Time to Buy and Sell Stock IV") {
    Solution s;

    vector<Segment> seg = { { 0, 2 }, { 3, 5 }, { 6, 7 } };
    CHECK(s.prices_to_segment({ 1, 2, 3, 0, 10, 10, 5, 6 }) == seg);

    CHECK(s.maxProfit(0, {}) == 0);
    CHECK(s.maxProfit(0, { 2 }) == 0);
    CHECK(s.maxProfit(2, {}) == 0);

    CHECK(s.maxProfit(1, { 2, 5, 0, 1, 5, 4, 1, 7 }) == 7);
    CHECK(s.maxProfit(3, { 2, 5, 0, 1, 5, 4, 1, 7 }) == 14);
    CHECK(s.maxProfit(2, { 2, 5, 0, 1, 5, 4, 1, 7 }) == 11);

    CHECK(s.maxProfit(1, { 1, 2, 3, 4, 3, 4, 5, 6, 7, 8 }) == 7);
    ...
    CHECK(s.maxProfit(100, prices) == 8740);
}
#endif
